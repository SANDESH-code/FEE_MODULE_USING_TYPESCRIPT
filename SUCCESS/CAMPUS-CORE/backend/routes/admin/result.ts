...
// Update the import path and name below to match your actual Course model file and export
// Update the path and export below to match your actual Course model file and export
// Update the path below to the actual location of your Course model file
// Update the import below to the correct path and export for your Course model
// Example: import CourseModel from '../../models/course.model';
// Update the path below to the actual location of your Course model file
// Example: import CourseModel from '../../models/Course';
import CourseModel from '../../student/Course';
  ...
  try {
    ...
    if (course_results) {
      for (const cr of course_results) {
        ...
        const course = await CourseModel.findById(cr.course_id);
        if (!course) {
...
router.get('/:resultId', admin_only, async (req: CustomRequest, res: Response) => {
  try {
    ...
    res.status(200).json({
      ...
      student_id: resultRecord.student_id?.toString(),
